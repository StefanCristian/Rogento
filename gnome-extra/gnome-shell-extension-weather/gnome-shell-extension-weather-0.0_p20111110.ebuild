...
src_compile() {
	...
	cd "${S}" || die 'Failed to cd into folder'
	./autogen.sh --prefix=/usr || die 'Failed to autogen'
	emake
}
...
src_install() {
	...
	mv weather-extension-configurator{.py,} || die 'Failed to mv the file'
	dobin "${S}/weather-extension-configurator"
